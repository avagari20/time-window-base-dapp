...
const source = await readFile(resolve(process.cwd(), "contracts/TimeWindow.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TimeWindow.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TimeWindow.sol"]?.TimeWindow;
if (!contract) {
...
console.log(`Deploying TimeWindow from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TIME_WINDOW_CONTRACT_ADDRESS to this contract address in Vercel.");
